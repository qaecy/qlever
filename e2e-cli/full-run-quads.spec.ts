...
describe('QLever CLI E2E Flow Quads', { timeout: 120000 }, () => {
    ...
    it('should add a triple via SPARQL UPDATE', () => {
        ...
        const out = execDocker(`/workspace/qlever-cli update ${CONTAINER_DB_BASE} "${updateQuery}"`);
        const result = JSON.parse(out);
        expect(result.success).toBe(true);

        // Verify
        const verifyOut = execDocker(`/workspace/qlever-cli query ${CONTAINER_DB_BASE} "SELECT ?s WHERE { GRAPH <http://example.org/g3> { ?s ?p ?o } }" csv`);
    ...
    it('should write a triple and specify graph using a flag', () => {
        ...
        const out = execDocker(`/workspace/qlever-cli write ${CONTAINER_DB_BASE} nt - --graph http://example.org/g4`, triple);
        const result = JSON.parse(out);
        expect(result.success).toBe(true);

        // Verify
        const verifyOut = execDocker(`/workspace/qlever-cli query ${CONTAINER_DB_BASE} "SELECT ?s WHERE { GRAPH <http://example.org/g4> { ?s ?p ?o } }" csv`);
    ...
    it('should still have the correct data after binary-rebuild', () => {
        const out = execDocker(`/workspace/qlever-cli query ${CONTAINER_DB_BASE}.rebuilt "SELECT ?s ?p ?o WHERE { ?s ?p ?o }" csv`);
        const lines = out.trim().split('\n');
