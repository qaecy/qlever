...
describe('QLever CLI Extended Commands', { timeout: 120000 }, () => {
    ...
    const { execDocker, execDockerRaw } = createExecHelpers(CONTAINER_CWD, WORKSPACE_DIR);
        ...
        const out = execDocker(`/workspace/qlever-cli build-index '${configData.replace(/'/g, "'\\''")}'`);
    ...
    it('should return stats as valid JSON', () => {
        const out = execDocker(`/workspace/qlever-cli stats ${CONTAINER_DB_BASE}`);
    ...
    it('should execute a query via query-json', () => {
        ...
        const out = execDocker(`/workspace/qlever-cli query-json '${input.replace(/'/g, "'\\''")}'`);
        expect(out).toContain('count');
    });

    it('should fail query-json with missing fields', () => {
        const input = JSON.stringify({ query: "SELECT * WHERE { ?s ?p ?o }" });
        const { exitCode, stderr } = execDockerRaw(`/workspace/qlever-cli query-json '${input.replace(/'/g, "'\\''")}'`);
        expect(exitCode).not.toBe(0);
        expect(stderr).toContain('Missing required fields');
    });

    it('should fail query-json with invalid JSON', () => {
        const { exitCode, stderr } = execDockerRaw(`/workspace/qlever-cli query-json 'not valid json'`);
        expect(exitCode).not.toBe(0);
    ...
    it('should serialize the index as nt to stdout', () => {
        const out = execDocker(`/workspace/qlever-cli serialize ${CONTAINER_DB_BASE} nt`);
    ...
    it('should serialize the index as nt to a file', () => {
        ...
        execDocker(`/workspace/qlever-cli serialize ${CONTAINER_DB_BASE} nt ${outputFile}`);
    ...
    it('should serialize the index as nq to stdout', () => {
        const out = execDocker(`/workspace/qlever-cli serialize ${CONTAINER_DB_BASE} nq`);
        expect(out).toContain('<http://example.org/s1>');
    });
    ...
    it('should execute CONSTRUCT query to file', () => {
        ...
        const out = execDocker(`/workspace/qlever-cli query-to-file ${CONTAINER_DB_BASE} "${query}" nt ${outputFile}`);
        const result = JSON.parse(out);
    ...
    it('should execute CONSTRUCT query to stdout', () => {
        ...
        const out = execDocker(`/workspace/qlever-cli query ${CONTAINER_DB_BASE} "${query}" nt`);
        expect(out).toContain('<http://example.org/s1>');
        expect(out).toContain('<http://example.org/p1>');
    });
    ...
    it('should handle empty stdin for write (0 triples)', () => {
        const out = execDocker(`/workspace/qlever-cli write ${CONTAINER_DB_BASE} nt -`, '');
        const result = JSON.parse(out);
        expect(result.success).toBe(true);
    ...
    it('should handle empty stdin for delete (0 triples)', () => {
        const out = execDocker(`/workspace/qlever-cli delete ${CONTAINER_DB_BASE} nt -`, '');
        const result = JSON.parse(out);
        expect(result.success).toBe(true);
    ...
    it('should fail write with unsupported format', () => {
        const { exitCode, stderr } = execDockerRaw(`/workspace/qlever-cli write ${CONTAINER_DB_BASE} xml -`, '');
        expect(exitCode).not.toBe(0);
    ...
    it('should skip binary-rebuild when no deltas exist', () => {
        ...
        execDocker(`/workspace/qlever-cli build-index '${config.replace(/'/g, "'\\''")}'`);

        const out = execDocker(`/workspace/qlever-cli binary-rebuild ${freshDir}/fresh-index`);
        const result = JSON.parse(out);
        expect(result.success).toBe(true);
    ...
    it('should rebuild to an explicit output basename, leaving original unchanged', () => {
        ...
        execDocker(`/workspace/qlever-cli write ${rebuildDir}/rb-index nt -`, newTriple);

        // Rebuild to an explicit output path
        const outputBase = `${rebuildDir}/rb-index.rebuilt`;
        const out = execDocker(`/workspace/qlever-cli binary-rebuild ${rebuildDir}/rb-index ${outputBase}`);
        const result = JSON.parse(out);
        expect(result.success).toBe(true);
        expect(result.message).toContain('Binary rebuild completed successfully');

        // Rebuilt index contains both triples
        const queryOut = execDocker(`/workspace/qlever-cli query ${outputBase} "SELECT ?s WHERE { ?s ?p ?o }" csv`);
        ...
        const origOut = execDocker(`/workspace/qlever-cli query ${rebuildDir}/rb-index "SELECT ?s WHERE { ?s ?p ?o }" csv`);
    ...
    it('should rebuild in place, overwrite original files, and clear update-triples', () => {
        ...
        execDocker(`/workspace/qlever-cli write ${rebuildDir}/ip-index nt -`, newTriple);

        // Rebuild in place (no output basename)
        const out = execDocker(`/workspace/qlever-cli binary-rebuild ${rebuildDir}/ip-index`);
        const result = JSON.parse(out);
        expect(result.success).toBe(true);
        expect(result.message).toContain('Binary rebuild completed successfully');
        ...
        const queryOut = execDocker(`/workspace/qlever-cli query ${rebuildDir}/ip-index "SELECT ?s WHERE { ?s ?p ?o }" csv`);
    ...
    it('should fail gracefully with non-existent index for query', () => {
        const { exitCode, stderr } = execDockerRaw(`/workspace/qlever-cli query ${CONTAINER_CWD}/does-not-exist "SELECT * WHERE { ?s ?p ?o }"`);
        expect(exitCode).not.toBe(0);
        expect(stderr.length).toBeGreaterThan(0);
    });

    it('should fail gracefully with non-existent index for stats', () => {
        const { exitCode, stderr } = execDockerRaw(`/workspace/qlever-cli stats ${CONTAINER_CWD}/does-not-exist`);
        expect(exitCode).not.toBe(0);
        expect(stderr.length).toBeGreaterThan(0);
    });

    it('should fail gracefully with invalid format for query', () => {
        const { exitCode, stderr } = execDockerRaw(`/workspace/qlever-cli query ${CONTAINER_DB_BASE} "SELECT * WHERE { ?s ?p ?o }" xml`);
        expect(exitCode).not.toBe(0);
        expect(stderr).toContain('Unsupported format');
    });

    it('should fail gracefully with invalid format for serialize', () => {
        const { exitCode, stderr } = execDockerRaw(`/workspace/qlever-cli serialize ${CONTAINER_DB_BASE} ttl`);
        expect(exitCode).not.toBe(0);
    ...
    it('should print help and exit 0', () => {
        const { exitCode, stdout } = execDockerRaw(`/workspace/qlever-cli --help`);
        expect(exitCode).toBe(0);
        expect(stdout).toContain('Commands:');
    });

    it('should fail with unknown command', () => {
        const { exitCode, stderr } = execDockerRaw(`/workspace/qlever-cli nonsense`);
        expect(exitCode).not.toBe(0);
    ...
    it('should write a materialized view and return success JSON', () => {
        ...
        const out = execDocker(`/workspace/qlever-cli write-view ${CONTAINER_DB_BASE} sppairs "${writeQuery}"`);
        const result = JSON.parse(out);
        expect(result.success).toBe(true);
    ...
    it('should query using a written materialized view', () => {
        ...
        const out = execDocker(`/workspace/qlever-cli query ${CONTAINER_DB_BASE} "${viewQuery}" csv`);
        expect(out).toContain('http://example.org/s1');
        expect(out).toContain('http://example.org/p1');
    ...
    it('should explicitly load a written materialized view', () => {
        const out = execDocker(`/workspace/qlever-cli load-view ${CONTAINER_DB_BASE} sppairs`);
        const result = JSON.parse(out);
        expect(result.success).toBe(true);
        expect(result.viewName).toBe('sppairs');
        expect(result.message).toContain('sppairs');
    ...
    it('should fail to load a non-existent materialized view', () => {
        const { exitCode, stderr } = execDockerRaw(`/workspace/qlever-cli load-view ${CONTAINER_DB_BASE} no-such-view`);
        expect(exitCode).not.toBe(0);
