...
void GzipOutputStream::write(const std::string& data) {
  ...
  int result =
      gzwrite(static_cast<gzFile>(gzFile_), data.c_str(), data.length());
  if (result <= 0) {
...
bool GzipOutputStream::isOpen() const { return gzFile_ != nullptr; }
...
void ProgressTracker::logProgress(size_t currentItems,
  ...
  double itemsPerSecond =
      elapsedSeconds > 0 ? currentItems / static_cast<double>(elapsedSeconds)
                         : 0;

  std::string eta = "";
  if (itemsPerSecond > 0) {
    double minutesElapsed = elapsedSeconds / 60.0;
    eta = " (" + std::to_string(static_cast<int>(minutesElapsed)) +
          "min elapsed)";
  }
...
double ProgressTracker::getItemsPerSecond(size_t currentItems) const {
  ...
  return elapsedSeconds > 0 ? currentItems / static_cast<double>(elapsedSeconds)
                            : 0;
}

std::chrono::seconds ProgressTracker::getElapsedTime() const {
  auto currentTime = std::chrono::steady_clock::now();
  return std::chrono::duration_cast<std::chrono::seconds>(currentTime -
                                                          startTime_);
}
...
RdfOutputWriter::RdfOutputWriter(const std::string& format,
...
void RdfOutputWriter::writeTriple(const std::string& subject,
                                  const std::string& predicate,
                                  const std::string& object) {
  std::string formattedTriple =
      RdfFormatUtils::formatTriple(subject, predicate, object, format_);
  writeRawTriple(formattedTriple);
}

void RdfOutputWriter::writeQuad(const std::string& subject,
                                const std::string& predicate,
                                const std::string& object,
                                const std::string& graph) {
  std::string formattedQuad =
      RdfFormatUtils::formatQuad(subject, predicate, object, graph, format_);
...
std::string DatabaseSerializer::extractValue(
    const nlohmann::json& binding) const {
...
void DatabaseSerializer::serialize(const std::string& format,
                                   const std::string& outputFile) {
  // Validate format
  if (!RdfFormatUtils::isValidFormat(format)) {
    throw std::runtime_error("Invalid format: " + format +
                             ". Supported formats: nt, nq");
  }
