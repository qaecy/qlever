...
IndexStatsCollector::IndexStatsCollector(
...
nlohmann::json IndexStatsCollector::collectStats(
    const std::string& indexBasename) const {
  ...
  std::cerr << "Gathering index statistics for " << indexBasename << "..."
            << std::endl;
  ...
  runStatsQuery(
      response, "blankNodeSubjects",
      "SELECT (COUNT(*) AS ?count) WHERE { ?s ?p ?o . FILTER(isBlank(?s)) }");
  runStatsQuery(
      response, "blankNodeObjects",
