...
ParallelFileBuffer::ParallelFileBuffer(size_t blocksize,
  ...
  // Support "-" as a special filename meaning stdin so that `build-index`
  // (and other CLI commands) can read RDF data piped to stdin instead of a
  // regular file. `/dev/stdin` is not reliably fopen()-able on Alpine Docker.
  if (filename == "-") {
